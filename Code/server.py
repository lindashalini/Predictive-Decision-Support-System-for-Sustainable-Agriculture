...
# ML Models (will be loaded if available)
...
def parse_arduino_data(line):
    ...
    try:
        ...
        if "Soil Moisture:" in line:
            ...
            # Complete data point received, store it and emit
...
def predict_irrigation():
    ...
    Required input: CropDays, SoilMoisture, temperature, Humidity
    """
...
def predict_crop():
    ...
    Required input: temperature, humidity, ph, rainfall
    """
...
def connect_arduino():
    """Manually connect to Arduino with specified port"""
    data = request.get_json()
    port = data.get('port')

    if start_serial_connection(port):
        return jsonify({'status': 'connected', 'port': port})
    else:
        return jsonify({'status': 'failed', 'port': port}), 500
...
if __name__ == '__main__':
    print("=" * 50)
    print("🌱 Predictive Soil Monitoring Server")
    print("=" * 50)
